...
import re
import yaml
import random  # Добавлен импорт для random.shuffle в smart_truncate_chunk
from datetime import datetime
from pathlib import Path  # Добавляем импорт Path
from typing import List, Dict, Union, Iterator, Optional  # Добавил недостающие типы
...
class OpenAI_API:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            auto_check_connection: Автоматически проверять соединение при инициализации
        """
        ...
        if not self.api_key:
            raise ValueError("API ключ не найден в config.yaml")
        ...
        # Проверка соединения только если запрошена
    ...
    def check_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Ошибка проверки подключения: {response.status_code} - {response.text}")
                return False
        except Exception as e:
    ...
    def _extract_context_limit(self, error_message: str) -> int:
        ...
        # Формат ошибки OpenAI
        match = re.search(r"maximum context length is (\d+) tokens", error_message)
        if match:
            limit = int(match.group(1))
            logger.info(f"Найден лимит контекста: {limit}")
            return max(1000, int(limit * 0.9))
        
        # Альтернативный формат OpenAI
        match = re.search(r"This model's maximum context length is (\d+) tokens", error_message)
        if match:
            limit = int(match.group(1))
            logger.info(f"Найден лимит контекста (альтернативный формат): {limit}")
            return max(1000, int(limit * 0.9))
        
        logger.warning("Не удалось определить лимит из сообщения об ошибке, используем 8000")
        return 8000  # Для OpenAI используем больший стандартный лимит
    ...
    def _handle_context_overflow(self, response, api_url, headers, payload):
        ...
        """
        # Для OpenAI API извлекаем сообщение об ошибке из JSON
        try:
        ...
        try:
            ...
            logger.error(f"Детальная ошибка OpenAI API: {stack_trace}")
    ...
    def query(self, messages: Union[List[Dict], str], 
        ...
        # Используем модель из конфига или по умолчанию gpt-4o
        if model is None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return content
            elif response.status_code == 400 and "context_length_exceeded" in response.text:
                ...
                if maintain_history and not content.startswith("Error:"):
                    ...
                    content_without_warning = re.sub(r'^⚠️ ВНИМАНИЕ:[^\n]*\n\n', '', content)
    ...
    def query_stream(self, messages: Union[List[Dict], str], 
        ...
        """Потоковая передача ответов от API."""
        api_url = f"{self.base_url}/chat/completions"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                yield "⚠️ ВНИМАНИЕ: Контекст слишком большой. Сохранены только системное сообщение и последний запрос.\n\n"
                ...
                if total_chars / 4 > max_tokens and new_messages:
                    ...
                    if len(new_messages[0]["content"]) / 4 > max_tokens:
                        ...
                        new_messages[0]["content"] = self.smart_truncate_chunk(new_messages[0]["content"], max_chars)
                ...
                if retry_response.status_code == 200:
                    ...
                    yield f"Error: Не удалось обработать запрос даже после обрезки контекста: {retry_response.status_code}"
            else:
                yield f"Error: API response {response.status_code} - {response.text[:200]}"
        except Exception as e:
    ...
    def query_with_retry(self, messages: Union[List[Dict], str], 
        ...
        Args:
            ...
            model: Model identifier (default from config or gpt-4o)
...
def save_dialog(dialog_history: List[Dict], output_dir: Optional[Path] = None) -> str:
    """
...
def main():
    """Основная функция для интерактивного CLI."""
    # Parse command line arguments
    import argparse
    parser = argparse.ArgumentParser(description="OpenAI API CLI")
    parser.add_argument("--model", default=None, help="OpenAI модель (gpt-4o, gpt-3.5-turbo)")
    parser.add_argument("--system", default="You are a helpful assistant specializing in organizing and structuring information.", 
                       help="System message")
    ...
    try:
        api = OpenAI_API(
            ...
            auto_check_connection=True  # В CLI нужна проверка соединения
        )
    except ValueError as e:
        print(f"Ошибка: {e}")
        print("Проверьте наличие API ключа OpenAI в config.yaml")
        return
    ...
    # Check connection автоматически выполнялась в конструкторе
    
    model = args.model if args.model else api.config.get('openai_model', 'gpt-4o')
    
    print("\n🤖 OpenAI Chat")
    print(f"Модель: {model}")
    print(f"System: \"{args.system}\"")
    print(f"Температура: {api.temperature}")
    print("\nКоманды:")
    print("  [save] - Сохранить историю диалога")
    ...
    while True:
        ...
        if user_input.lower() == "[exit]":
            ...
            file_path = save_dialog(api.message_history)
            print(f"Диалог сохранен в {file_path}")
            continue
